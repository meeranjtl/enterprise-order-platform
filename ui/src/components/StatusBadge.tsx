...
const STATUS_STYLES: Record<string, string> = {
  ...
  OUT_OF_STOCK: 'bg-amber-500/15 text-amber-600 dark:text-amber-400',
  ...
  PAYMENT_APPROVED: 'bg-blue-500/15 text-blue-600 dark:text-blue-400',
  // indigo — in transit
  ...
  PAYMENT_REJECTED: 'bg-red-500/15 text-red-600 dark:text-red-400',
  DISCONTINUED: 'bg-red-500/15 text-red-600 dark:text-red-400',
...
export function StatusBadge({ status, className }: { status: string; className?: string }) {
  ...
  return (
    <span
      className={cn('inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium', style, className)}
    >
